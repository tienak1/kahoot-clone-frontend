    ...
    return (
        <main>
            <div className={`${cssStyle["form-container"]}`}>
                <h2 className={`${cssStyle["form-title"]}`}>Profile</h2>
                <img
                    className={`${cssStyle["ava-img"]}`}
                    src={avaTemplate}
                    alt="Avatar"
                ></img>
                <h2 className={`${cssStyle["name-text"]}`}>{user.fullname}</h2>
                <p className={`${cssStyle["email-text"]}`}>
                    Email: {user.email}
                </p>
                {currentForm === "name" ? (
                    <ChangeNameForm onFormSwitch={toggleForm} />
                ) : (
                    <ChangePassForm onFormSwitch={toggleForm} />
                )}
            </div>
        </main>
    );
