					...
					{screen.active ? (
						<div style={{
							...
							{selectedSlide.slideOrder - 1 >= 0 ? (
								<Button onClick={() => {
									const leftSlide = presentation.slides[selectedSlide.slideOrder - 1]
									handleSlideShowNextSlide(leftSlide)
								}}>
									<ArrowCircleLeftIcon sx={{ color: "#F69351", fontSize: "2rem" }}></ArrowCircleLeftIcon>
								</Button>
							) : (
								<Button disabled='true'/>
							)}
							
							<SlideShow slide={selectedSlide}>
							</SlideShow>

							{selectedSlide.slideOrder + 1 < presentation.slides.length ? (
								<Button onClick={() => {
									if (selectedSlide.slideOrder + 1 < presentation.slides.length) {
										const rightSlide = presentation.slides[selectedSlide.slideOrder + 1]
										handleSlideShowNextSlide(rightSlide)
									}
								}}>
									<ArrowCircleRightIcon sx={{ color: "#F69351", fontSize: "2rem" }}></ArrowCircleRightIcon>
								</Button>
							) : (
								<Button disabled='true'/>
							)}
